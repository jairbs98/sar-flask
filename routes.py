...
def versiones_recinto(recinto_id):
    ...
    return render_template(
        "theater_versions/form_create_theater_version.html",
        detalles_recinto=detalles_recinto,
...
def crear_zona(id_version):
    ...
    return render_template(
        "theater_areas/form_create_theater_area.html", detalles_recinto=detalles_recinto
    )
...
def detalle_zonas(id_zona):
    ...
    secciones, pagination = get_paginated_data(
        10, obtener_secciones_listado, obtener_total_secciones, id_zona
    )

    data = {"secciones": secciones, "pagination": pagination}
    return render_template(
        "theater_areas/config_theater_areas.html",
        detalles_zona=detalles_zona,
        svg_recinto_por_zona=svg_recinto_por_zona,
        data=data,
    )
...
def detalle_seccion(id_seccion):
    ...
    return render_template(
        "theater_sections/config_theater_sections.html",
        detalles_seccion=detalles_seccion,
    )
...
def crear_seccion(id_zona):
    ...
    return render_template(
        "theater_sections/form_create_theater_sections.html",
        detalles_seccion=detalles_seccion,
    )
...
def detalle_recinto(recinto_id):
    detalles_recinto = obtener_detalles_recinto(recinto_id)
    versiones, pagination = get_paginated_data(
        10, obtener_versiones_listado, obtener_total_versiones, recinto_id
    )
    pag_v = {"versiones": versiones, "pagination": pagination}
    return render_template(
        "theater/config_theater.html", detalles_recinto=detalles_recinto, pag_v=pag_v
    )


def detalle_version_recinto(version_id):
    detalles_version_recinto = obtener_detalles_version_recinto(version_id)
    zonas, pagination = get_paginated_data(
        10, obtener_zonas_listado, obtener_total_zonas, version_id
    )

    data_zonas = {"zonas": zonas, "pagination": pagination}
    return render_template(
        "theater_versions/config_theater_versions.html",
        detalles_version_recinto=detalles_version_recinto,
...
def editar_recinto(recinto_id):
    ...
    if request.method == "POST":
        ...
        if (
            nuevo_nombre != detalles_recinto["nombre"]
            or nueva_direccion != detalles_recinto["direccion"]
            or nuevo_mapa != detalles_recinto["mapa"]
        ):
...
def editar_zona(id_zona):
    ...
    if request.method == "POST":
        ...
        if (
            nuevo_nombre != detalles_zona["nombre_zona"]
            or nuevo_color != detalles_zona["color_zona"]
            or nuevo_estatus != detalles_zona["estatus_zona"]
        ):
...
def index():
    recintos, pagination = get_paginated_data(
        5, obtener_recintos_listado, obtener_total_recintos
    )

    pag = {"recintos": recintos, "pagination": pagination}

    return render_template("theater/theaters.html", pag=pag)
